...
\begin{frame}{Details of the time-resolved x-ray experiment}
  \begin{columns}[t]
    \column{0.57\textwidth}
      \begin{block}{What was done?}
        \begin{itemize}
         \item pump: \textit{vis}-fs-laser 
         \item probe: x-ray free electron laser (K edge of O-atom) 
        \end{itemize}
...
\begin{frame}{Generalized Langevin Oscillator}
  \begin{columns}[c]
    \column{0.45\textwidth}
	  \vspace*{.5cm}
% 	  \begin{figure}
		\includegraphics[width=1.1\textwidth]{figures/GLO_eqs.png}
% 	  \end{figure}
	\column{.55\textwidth}
% 	  \begin{figure}
...
%       \item[$\Rightarrow$] 
      \newline \textbf{despite} it being a local \textbf{maximum}!
      \item[$\Rightarrow$] dynamical trapping (see also 30 ps)
      \item effect dependent on fluence
      \item[$\Rightarrow$] consistent with experiment \newline(less ``precursor'' for low fluence)
    \end{itemize}   
  \end{block}
