...
\section{Introduction}
  \subsection{Motivation - in general and system specific}
...
\begin{frame}{Specific motivation for the CO/Ru-System}
  ...
  \begin{block}{Experimentally well studied system}
    \begin{itemize}
	  \item especially regarding fs-laser irradiation  e.g.:\newline Bonn \textit{et al.}, \textit{Science} 1999; Funk \textit{et al.}, \textit{JChemPhys} 2000 (both Ertl group - chemistry Nobel prize 2007).
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 	
	%modify number (2) to: Non-adiabatic coupling / ``electronic friction'' 
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 	
  \end{figure}
  ...
  \begin{block}{Coupling between three different kinds of degrees of freedom:} 
	...
	\begin{itemize}
	 ...
	 \item low heat capacity $\Rightarrow$ high temperatures ($\approx$5-10$\,$kK)
	\end{itemize}
...
\begin{frame}{Details of the time-resolved x-ray experiment}{Dell'Angela \textit{et al.}, \textit{Science} 2013 {\tiny (including Nilsson group, SLAC, Stanford)}}
  \vspace*{-.7cm}
  \begin{columns}[t]
...
%       \end{block}	  
  ...
  \end{figure}
         \vspace*{-.15cm}
%   \begin{figure}
    \includegraphics[width=1\textwidth]{figures/XrayIntOverTime.png}
%   \end{6figure}
  \end{columns}
\end{frame}

\begin{frame}{Details of the accompanying theory}{still Dell'Angela \textit{et al.}, \textit{Science} 2013 {\tiny (theory part by N\o{}rskov group, SUNCAT, Stanford)}}
  \begin{figure}
	\includegraphics[width=.4\textwidth]{figures/sciencePMF.png}
  \end{figure}
\end{frame}
...
\subsection{Foundations: 6D potential and two-temperature model}
...
\subsection{Electronic friction: non-adiabatic coupling approximated}
\begin{frame}{Non-adiabatic coupling}
  \begin{figure}
...
\subsection{The generalized Langevin oscillator (GLO)}
...
\subsection{``Half-time'': short summary {\footnotesize(and time for any first questions)}}
...
\begin{frame}{Anisotropic diffusion + dynamical trapping}
